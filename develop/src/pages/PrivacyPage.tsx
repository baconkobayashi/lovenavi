  ...
  return (
    ...
    >
      <div className="border-b border-black/10 px-6 py-5 text-[14px] leading-[1.8] text-ink-secondary sm:px-8">
        ●●●●株式会社（以下「当社」といいます）は、「lovenavi」（以下「本サービス」といいます）において、ユーザーの個人情報の取り扱いを以下のとおり定めます。本サービスの利用にあたり、本ポリシーをご確認のうえご同意ください。
      </div>
      ...
      <Section num="5" title="Cookieおよびアクセス解析">
        ...
        <p>
          また、サービス改善のためアクセス解析ツール（●●●●）を利用する場合があります。収集される情報は匿名化され、個人を特定するものではありません。
        </p>
      </Section>
      ...
      <Section num="6" title="情報の保存期間">
        <DataTable
          headers={['情報の種類', '保存期間']}
          rows={[
            ['アカウント情報', '退会から●年間（法令上の保存義務がある場合はその期間）'],
            ['生成履歴・行動ログ', '最終利用日から●年間'],
            ['AI学習に使用したデータ', '学習完了後も統計的なモデルに含まれる場合があります'],
            ['お問い合わせ記録', '対応完了から●年間'],
          ]}
        />
      </Section>
      ...
      <Section num="7" title="ユーザーの権利">
        <p>ユーザーは以下の権利を有します。お問い合わせ窓口よりご連絡ください。</p>
        <ul>
          <li>
            <strong>開示請求</strong>：保有する個人情報の開示を求めることができます
          </li>
          <li>
            <strong>訂正・追加・削除</strong>
            ：情報に誤りがある場合、訂正・削除を求めることができます
          </li>
          <li>
            <strong>利用停止・消去</strong>：個人情報の利用停止または消去を求めることができます
          </li>
          <li>
            <strong>AI学習への利用停止</strong>：行動ログのAI学習への利用停止を求めることができます
          </li>
          <li>
            <strong>アカウント削除</strong>
            ：マイページまたはお問い合わせよりアカウントの削除を請求できます
          </li>
        </ul>
        <Highlight>
          ご請求にあたっては、本人確認をさせていただく場合があります。対応期間は受付から原則●営業日以内とします。
        </Highlight>
      </Section>
